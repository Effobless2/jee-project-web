...
export class ShopSearchBarComponent {
  ...
  @Input() callback: (shops: Trade[]) => any = (_) => { };
  @Input() rollback: () => any = () => {};
  ...
  geoPositionCallback(p: Position) {
    const pos = this.positionWrapper(p);
  ...
  geoLocationCallback(res: GeocoderResult[]) {
    if (res.length === 0) {
      ...
      const pos = this.geocoderResultWrapper(res[0]);
      this.sendRequest(pos);
    }
  }

  sendRequest(p?: {lng: number, lat: number}) {
    const searchFilters: SearchShopFormularFields = this.formGroup.value as SearchShopFormularFields;
