...
export class GeocodingService{
    ...
    findLocation(parameter: string|{lat: number, lng: number}, callback: (locations: GeocoderResult[]) => void, error?: (status: string) => void){
        if (!this.geocoder) 
            this.geocoder = new google.maps.Geocoder()
        let options = typeof(parameter) === "string" ?
        {
            address: parameter
        } :
        {
