...
export class TradeSearchService {
  private controllerUrl: string = `${environment.beererApiUrl}trades/search`;

  constructor(private httpService: HttpService){}

  get(data: TradeSearchData, callback: (result: Trade[]) => any, error?: (err: HttpErrorResponse) => any){
    return this.httpService.get(`${this.controllerUrl}?name=${data.name ?? "" }&types=${data.types ?? "" }&lng=${data.lng ?? ""}&lat=${data.lat ?? ""}`)
      .subscribe({
        next: callback,
        error: error
      });
